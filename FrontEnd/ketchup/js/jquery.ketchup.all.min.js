...
jQuery.ketchup.validation("required","El campo es requerido.",function(g,b,c){g=b.attr("type").toLowerCase();return g=="checkbox"||g=="radio"?b.attr("checked")==true:c.length!=0}).validation("minlength","El campo debe de tener minimo {arg1} caracteres.",function(g,b,c,d){return c.length>=+d}).validation("maxlength","El campo debe de tener máximo {arg1} caracteres.",function(g,b,c,d){return c.length<=+d}).validation("rangelength","Este campo debe de tener una longitud entre {arg1} y {arg2}.",function(g,
b,c,d,h){return c.length>=d&&c.length<=h}).validation("min","Este campo al menos debe tener  {arg1}.",function(g,b,c,d){return this.isNumber(c)&&+c>=+d}).validation("max","Este campo no puede ser mayor que {arg1}.",function(g,b,c,d){return this.isNumber(c)&&+c<=+d}).validation("range","Debe estar entre {arg1} y {arg2}.",function(g,b,c,d,h){return this.isNumber(c)&&+c>=+d&&+c<=+h}).validation("number","Debe ser un número.",function(g,b,c){return this.isNumber(c)}).validation("digits","Debe serdigits.",function(g,b,c){return/^\d+$/.test(c)}).validation("email",
"El correo electrónico debe ser válido.",function(g,b,c){return this.isEmail(c)}).validation("url","Debe ser una URL válida.",function(g,b,c){return this.isUrl(c)}).validation("username","El usuario debe ser valido",function(g,b,c){return this.isUsername(c)}).validation("match","Debe ser {arg1}.",function(g,b,c,d){return b.val()==d}).validation("contain","Debe de contener {arg1}",function(g,b,c,d){return this.contains(c,d)}).validation("date","Debe tener una fecha válida.",function(g,b,c){return this.isDate(c)}).validation("minselect",
...
function(g){return/^([a-zA-Z])[a-zA-Z_-]*[\w_-]*[\S]$|^([a-
  zA-Z])[0-9_-]*[\S]$|^[a-zA-Z]*[\S]$/.test(g)}).helper("isDate",function(g){return!/Invalid|NaN/.test(new Date(g))}).helper("inputsWithName",function(g,b){return $('input[name="'+b.attr("name")+'"]',g)}).helper("inputsWithNameNotSelf",function(g,b){return this.inputsWithName(g,b).filter(function(){return $(this).index()!=b.index()})}).helper("getKetchupEvents",function(g){g=g.data("events").ketchup;var b=[];for(i=0;i<g.length;i++)b.push(g[i].namespace);
